...
def run_add_session_data(self):
    ...
    #API call for adding description
...
def get_general_session_data(self):
    #In this function we are creating data dict for post processing
    '''
    General Data, include phone os, phone version ....
    '''
    session_status = None
    if self.status != "Pass":
    ...
    #add the non duration kpi to data
...
def wait_for_session_video_becomes_available(self):
    ...
    while time.time() < t_end:
        ...
        if status and ("video_duration_ms" in status):
            ...
            break
        logger.info("Waiting for video becomes available")
